...
def update_stat_tables(var_to_bin, n_clicks, good_bad_def_data, bins_settings_data, auto_bin_algo, 
        equal_width_method, equal_width_w, equal_width_nb, equal_freq_method, equal_freq_f, equal_freq_nb):
    triggered = ["auto_bin_refresh_button.n_clicks"]
    if triggered[0] == "predictor_var_ib_dropdown.value":
        bins_settings_dict = {'variable': [{'column': 'person_age', 'type': 'numerical', 'infoVal': -1, 'bins': 'none'}, {'column': 'loan_status', 'type': 'categorical', 'infoVal': -1, 'bins': 'none'}]}
        ...
        return stat_df
    else: # auto bin refresh button clicked
        good_bad_def = {'bad': {'numerical': [], 'categorical': [{'column': 'loan_status', 'elements': [1]}], 'weight': 1}, 'indeterminate': {'numerical': [], 'categorical': []}, 'good': {'weight': 1}}
        ...
        new_stat_df = stat_calculator.compute_summary_stat_table()
        
        # new_stat_df = pd.DataFrame([348, 2384, 2389, 91])
        
        return new_stat_df

print(update_stat_tables("person_age", 1, 1, 1, "equal frequency", "width", 15, 10, "num_bins", 10, 10))

# """
# Interactive Binning Page:
# Update statistical table (before & after) when user 
# either changes the variable to be binned, or, clicked on 
# refresh button in the automated binning panel
# """
# @app.callback(
#     [
...
#         return [[], [DataTable(new_stat_df, width=70)]]
...
# """
# Interactive Binning Page:
# Update statistical table (before & after) whe user clicked on
# the refresh button in the automated binning panel
# """
...
#     ],
# )
# def update_stat_tables_on_auto_bin_algo_change(
#         n_clicks, good_bad_def_data, bins_settings_data, var_to_bin, auto_bin_algo, 
#         equal_width_method, equal_width_w, equal_width_nb, equal_freq_method, equal_freq_f, equal_freq_nb
#     ):
#     good_bad_def = json.loads(good_bad_def_data)
